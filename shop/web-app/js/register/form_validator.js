// #import app/message/message.js

/**
 * 表单校验框架 <br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_1.html" target="_blank">快速开始示例</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_2.html" target="_blank">动态添加/删除字段规则</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_4.html" target="_blank">添加自定义校验方法</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_3.html" target="_blank">简单条件配置</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_5.html" target="_blank">服务器返回错误 errorFromServer</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_6.html" target="_blank">服务器返回提示 tipsFromServer</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_7.html" target="_blank">自定义事件 onFeildErrFire(最终全部校验遇到每个错误显示之前)</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_8.html" target="_blank">自定义事件 afterValidateOneField & beforeValidateOneField(每次单个校验触发遇到错误前后,如失去焦点)</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_9.html" target="_blank">自定义事件 afterValidateAll & beforeValidateAll(全部校验前后)</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_10.html" target="_blank">添加表现层方法 addDisplayFunc</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_11.html" target="_blank">添加事件函数 addEventFunc</a><br />
 * <a href="http://style.aliui.com/js/5v/app/form_validator/demo/demo_12.html" target="_blank">单独校验(validateOne)和全部校验(validateAllField)</a><br />
 * @module AE.form.validator
 */
AE.namespace('AE.form.validator');

/* @descript 深度merge 对象, 理论上支持无限层 */
AE.namespace('tools');
...
AE.form.validator = function(){
	...
	var defConfig = {
		...
		/**
		 * resize窗口是否需要重新定位提示元素
		 * @property config.ifNeedRepositionAdvice {boolean}
		 * @dafault true
		 */
		ifNeedRepositionAdvice: true,
	...
	_self.customEvent={
		/**
		 * 最终全部校验遇到每个错误显示之前
		 * @event onFeildErrFire
		 * @param errorSubmitTime {int} 表单校验出错总次数
		 * @param errFieldNum {int} 当前表单出错控件数量
		 * @param el {dom} 当前校验出错的dom
		 * @public
		*/
		 ...
		 * @param formObj {dom} 当前校验表单DOM
		 * @public
		*/
		'beforeValidateAll'      :new YAHOO.util.CustomEvent('beforeValidateAll',_self,false,YAHOO.util.CustomEvent.LIST),
		/**
		 * 最终全部校验之后
		 * @event afterValidateAll
		 * @param errorSubmitTime {int} 表单校验出错总次数
		 * @param errFieldNum {int} 当前表单出错控件数量
		 * @param finalResult {boolean} 表单校验最终结果
		 * @public
		*/
		'afterValidateAll'      :new YAHOO.util.CustomEvent('afterValidateAll',_self,false,YAHOO.util.CustomEvent.LIST),
		/**
		 * 每次单个校验触发遇到错误之前,如失去焦点
		 * @event beforeValidateOneField
		 * @param el {dom} 当前校验控件
		 * @public
		*/
		'beforeValidateOneField'	: new YAHOO.util.CustomEvent('beforeValidateOneField',_self,false,YAHOO.util.CustomEvent.LIST),
		/**
		 * 每次单个校验触发遇到错误之后,如失去焦点
		 * @event afterValidateOneField
		 * @param errorFieldId {string} 当前校验控件ID
		 * @param returnMsg {object} 当前控件校验返回集合
		 * @param returnMsg.result {boolean} 校验结果标志位
		 * @param returnMsg.msg {string} 出错信息
		 * @param returnMsg.validatorName {string} 校验器名称
		 * @public
		*/
		'afterValidateOneField' :new YAHOO.util.CustomEvent('afterValidateOneField',_self,false,YAHOO.util.CustomEvent.LIST),
		/**
		 * 触发无校验器事件之后
		 * @event afterFieldBlurNotValidators
		 * @param elId {string} 当前校验控件ID
		 * @param returnMsg {object} 当前控件校验返回集合
		 * @public
		*/
		...
		/**
		 * 新增校验规则之后
		 * @event onAfterAddValidateField
		 * @param fieldsConfig {object} 校验规则对象
		 * @public
		*/
		'onAfterAddValidateField':new YAHOO.util.CustomEvent('afterAddValidateField',_self,false,YAHOO.util.CustomEvent.LIST),
		/**
		 * 删除校验规则之后
		 * @event onAfterDelValidateField
		 * @param fieldId {String} 校验对象ID
		 * @public
		*/
	...
	_self.validatorsPool = {
		...
		//最小长度
		minLength : function(val, el, params) {return val.length >= parseInt(params[0], 10);},
		//最大长度
		maxLength : function(val, el, params) {return val.length <= parseInt(params[0], 10);},
		//最小值
	...
	*/
	...
	var methods = {
		'oneRequire': function(val, el, params){
			...
			if (els.length > 0) {
				var elsArray = toArray(els);
				for (var i=0; i < elsArray.length; i++) {
					if (elsArray[i].checked) {
						return true;
					}
				}
				return false;
	...
	/**
	 * 添加显示层方法(重写内部方法)
	 * @method addDisplayFunc
	 * @param {object} options 入参对象
	 * @param {function} options.showErrorMsg_showMsgFollowOtherObj 自动找到元素，跟随显示错误方法
	 * @param {function} options.getAdvice_getTips 取得tipsAdvice并存入_self.allTipsAdvisor
	 * @param {function} options.getAdvice_getError  取得ErrorAdvice并存入_self.allErrorAdvisor
	 * @param {function} options.showStatus_changeTitleClass  根据结果改变字段标题
	 * @param {function} options.showTipsMsg_showMsgFollowOtherObj   自动找到元素，跟随显示提示方法
	 * @param {function} options.closeTipsMsg_close  关闭提示信息_直接关闭
	 * @param {function} options.closeErrorMsg_close  关闭错误信息_直接关闭
	 * @public
	 */
	...
	_self.executeBeforeFunc = function(data_before){
		...
		if(elid in _self.configData.fields){
			vali = _self.configData.fields[elid].validators[elv];
		} else {
			return false;
		}
		el = get(elid);
	 ...
	 * @param {object} el 单独校验DOM对象
	 * @param {string} validators 校验器
	 * @param {boolean} byAllRule 强行进行所有规则校验
	 * @returns {object} returnMsg 返回结果集
	<pre>
		{
			{boolean} result 是否通过校验,
			{string} msg 出错信息,
			{string} validatorName 错误校验规则名称
		}
	</pre>
	 * @public
	 */
	_self.validateField = function(el, validators, o, byAllRule){
		_self.customEvent['beforeValidateOneField'].fire(el);
		var result = true;
		var msg = null;
	 ...
	 * @param {object} options 入参对象
	 * @param {string} options.elId 需要检测DOM id
	 * @param {boolean} options.showError 是否显示出错信息
	 * @returns {object} returnMsg 同validateField返回值
	 * @public
	 */
	_self.validateOneField = function(options){
		var el =  get(options.elId),
				showError = options.showError;

		// 开始校验
		var returnMsg = _self.validateField( el, _self.getValidators(el) );

		// 根据结果和配置调用显示错误方法
		if(returnMsg.result === false && showError === true){
			// 获得显示配置
			var displayFuncs = _self.configReader(el, 'displayFuncMap');
			// 根据结果改变字段标题
			...
			// 显示出错
			_self.displayFuncPool[displayFuncs['showErrorMsg']](el, advice, returnMsg.msg);
			// 滚动定位到出错地方
			_self.displayFuncPool['scrollToField'](el);
		}
...
//    	}
	...
	/**
	 * 校验所有字段
	 * @method validateAllField
	 * @return {boolean} finalResult 校验结果
	 * @public
	 */
	_self.validateAllField = function(){
		...
		// 校验之前调用
		_self.customEvent['beforeValidateAll'].fire(_self.formObj);
	...
	/**
	 * 初始化form - 自动绑定form submit事件
	 * @method init
	 * @param {string} formId 表单ID
	 * @param {object} userConfig 配置
	 * @param {string} userConfig.tipsAdviceClass 提示容器样式
	 * @param {string} userConfig.errorAdviceClass 出错容器样式
	 * @public
	 */
	...
	/**
	 * 初始化form - 仅初始化校验框架，不绑定表单事件
	 * @method initWithoutEvent
	 * @param {string} formId 表单ID
	 * @param {object} userConfig 配置说明同init
	 * @public
	 */
	_self.initWithoutEvent = function(formId, userConfig){
		...
		//窗口resize时重定位信息框
		if(_self.configData.ifNeedRepositionAdvice){
	 ...
	 * @param {dom} elId 校验控件对象
	 * @public
	 */
	_self.clearValidateFieldError = function(elId){
			var el = get(elId);
		var displayFuncs = _self.configReader(el, 'displayFuncMap');
		_self.displayFuncPool[displayFuncs['showStatus']](el, true);
		_self.displayFuncPool[displayFuncs['closeErrorMsg']](el);
	...
	*/
	...
	_self.addValidateField = function(fieldsConfig){
		_self.configData.fields = AE.tools.depthMerge(_self.configData.fields, fieldsConfig);
	...
	/**
	* 校验框架初始化后，动态移除校验字段
	* @method delValidateField
	* @param {String} fid 需移除校验字段的ID
	* @param {Boolean} isDelAll 对于radio、check等元素，当指定移除其中一个的校验时，会移除与其同组的元素（同name）的校验
	* @public
	*/
	_self.delValidateField = function(fid, isDelAll){
		var field = get(fid),
			objField = null;
		if(isDelAll){
			objField = {};
			objField[fid] = field; // 必须传入对象，{id: el}
			// 取消所有注册事件
			_self.fieldsEventHandle(objField, 'remove');
		} else {
